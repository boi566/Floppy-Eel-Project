# added null move pruning , iterative deepening , transposition tables and time management
import chess as c
import time
...
def evl(b , depth):
    ...
    return(score)
def lva(b , move):
    if b.is_capture(move) == False:
        return(0)
    else:
        nol = b.piece_at(move.from_square)
        nolval = VALS[nol.piece_type]
    if b.is_en_passant(move):
        nol2val = VALS[c.PAWN]
    else:
        nol2 = b.piece_at(move.to_square)
        nol2val = VALS[nol2.piece_type]
    return((nol2val * 10) - nolval)
def quench(b , alpha , beta , stme):
    if time.time() - stme > 5:
        return 0
    pat = evl(b , 0)
    if b.turn == c.WHITE:
       if pat >= beta:
           return(beta)
       alpha = max(alpha , pat)
       for move in sorted(b.generate_legal_captures() , key = lambda m: lva(b , m), reverse = True):
           ...
           if tget:
               ...
               if pat + vl + DELLA <= alpha:
                   continue
           b.push(move)
           score = quench(b , alpha , beta , stme)
           b.pop()
           if score >= beta:
               return(beta)
...
beta = float('inf')):
    ...
    if depth == 0:
        return(quench(b , alpha , beta , stme) , None)
    ...
    if key in tt:
        ...
        if entry['depth'] >= depth:
            ...
            if entry['flag'] == LBW and entry['score'] >= beta:
                return(beta , entry['move'])
            if entry['flag'] == UBW and entry['score'] <= alpha:
                return(alpha , entry['move'])
        fsd = entry['move']
    if depth >= 3 and not b.is_check():
        b.push(c.Move.null())
        score , _ = get(b , depth - 1 - 2 , stme , alpha , beta)
        b.pop()
        if b.turn == c.WHITE and score >= beta:
            return(beta , None)
        elif b.turn == c.BLACK and score <= alpha:
            return(alpha , None)
    leg = sorted(b.legal_moves , key = lambda m: lva(b , m) , reverse = True)
    if fsd and fsd in leg:
        leg.remove(fsd)
        leg.insert(0 , fsd)
    if b.turn == c.WHITE:
        maximum = float('-inf')
        ...
        for move in leg:
            b.push(move)
            score , _ = get(b , depth - 1 , stme , alpha , beta)
            b.pop()
            if score > maximum:
                maximum = score
                lowmove = move
        ...
        for move in leg:
            ...
            if beta <= alpha:
                break
    b_score = maximum if b.turn == c.WHITE else minimum
    if b_score <= og_alpha:
        flag = UBW
    elif b_score >= beta:
        flag = LBW
...
def itdep(b , max_depth):
    fleek = None
    stme = time.time()
    for i in range(1 , max_depth + 1):
        score , ml = get(b , i , stme)
        if time.time() - stme > 5:
            break
        if ml:
            fleek = ml
